...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Memeriksa apakah ada entri dengan tanggal hari ini
    $sql_check = "SELECT jumlah FROM hitung WHERE tanggal = '$today'";
    $result = $conn->query($sql_check);

    if ($result->num_rows > 0) {
        // Jika ada, update press_count
        $row = $result->fetch_assoc();
        $press_count = $row['jumlah'] + 1;

        $stmt = $conn->prepare("UPDATE hitung SET jumlah = ? WHERE tanggal = ?");
        $stmt->bind_param("is", $press_count, $today);
    } else {
        // Jika tidak ada, insert new row dengan press_count = 1
        $press_count = 1;

        $stmt = $conn->prepare("INSERT INTO hitung (tanggal, jumlah) VALUES (?, ?)");
        $stmt->bind_param("si", $today, $press_count);
    }
		...
		<div class="section">
			<div class="box-area">
                ...
                <!-- <section class="contact"> -->
                    <div class="container"> 
                        <div class = "video">
                            <iframe width="600" height="350" src="https://www.youtube.com/embed/fxe6qev-bno?si=Rl-hjnhRbHRugWZp" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                        </div>
                    </div>
      
                    <div class="kontrol">
                    <div class="button-box-manual">
                    <button type="button" id="btn-manual" class="button">Mode Manual</button></div>

                    <div class="button-box-otomatis">
                    <button type="button"id= "btn-otomatis" class="button">Mode Otomatis</button></div>

                    <div class="button-box-manual2">
                    <form action = "" method = "post">
                        <button type="submit" id= "btn-hijau">HIJAU</button>
                    </form>
                        <button type="button" id = "btn-merah">MERAH</button>
                    </div>

                    <div class="button-box-otomatis2"></div>
                    </div> 
                    <div class="informasi">
                    <img src = "dishub.png">
                    <p class="hijau">Tombol Hijau untuk menyalakan Lampu Penyeberangan!</p1>
                    <p class="merah">Tombol Merah untuk mematikan Lampu Penyeberangan!</p2>
                    </div>
                    </div>
